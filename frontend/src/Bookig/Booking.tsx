...
function Booking() {
  ...
  useEffect(() => {
    const fetchAllMovies = async () => {
      ...
      try {
        while (hasMore) {
          const response = await fetch(`http://localhost:5000/api/movie/getAllMoviesPagniation/${page}`);
          if (!response.ok) {
  ...
  const getCardSize = (index: number) => {
    if (index === 0) return "col-span-2 row-span-2"; // Large card for first movie
    if (index === 1) return "col-span-1 row-span-2"; // Medium card for second movie
    return "col-span-1 row-span-1"; // Small card for others
  };

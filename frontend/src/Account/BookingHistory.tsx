...
const BookingHistory: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = `http://localhost:5000/api/BookingHistory/getBookingHistory/${finalUserId}`;
      try {
  ...
  const fetchBookingDetail = async (bookingId: string) => {
    ...
    setModalError(null);
    const authToken = localStorage.getItem('authToken');
    const url = `http://localhost:5000/api/BookingHistory/getBookingHistoryDetail/${bookingId}`;

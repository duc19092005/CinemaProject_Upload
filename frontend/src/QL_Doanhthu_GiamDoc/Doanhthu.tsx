...
const RevenueList: React.FC = () => {
  ...
  const fetchRevenue = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/Revenue/GetAllRevenue', {
        method: 'GET',
        headers: {
          'Accept': '*/*',
  ...
  const fetchRevenueDetail = async (cinemaId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/Revenue/GetRevenueByCinemaId?cinemaId=${cinemaId}`, {
        method: 'GET',
        headers: {
          'Accept': '*/*',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,

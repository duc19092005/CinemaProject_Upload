...
const AddMovie: React.FC = () => {
    ...
    const fetchMovies = (page: number) => {
        fetchData(
            `http://localhost:5000/api/movie/getAllMoviesPagniation/${page}`,
            (data) => {
    ...
    useEffect(() => {
        ...
        fetchData(
            "http://localhost:5000/api/MovieGenre/GetMovieGenreList",
            setTheloaiOptions,
        ...
        fetchData(
            "http://localhost:5000/api/MinimumAge/GetMinimumAge",
            setAgeOptions,
            "Không thể tải danh sách độ tuổi"
        );
        fetchData(
            "http://localhost:5000/api/Language/GetLanguage",
            setLanguageOptions,
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const url = editIndex !== null
                ? `http://localhost:5000/api/movie/editMovie?movieID=${movies[editIndex].movieId}`
                : "http://localhost:5000/api/movie/createMovie";
    ...
    const handleEdit = async (index: number) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/movie/getMovieDetail/${movie.movieId}`, {
                headers: {
                    Authorization: `Bearer ${TOKEN}`,
                },
            });
    ...
    const confirmDelete = async () => {
        if (deleteIndex !== null) {
            ...
            if (movieId) {
                try {
                    await axios.delete(`http://localhost:5000/api/movie/DeleteMovie/${movieId}`, {
                        headers: {
                            Authorization: `Bearer ${TOKEN}`,
                            accept: '*/*'
                        },

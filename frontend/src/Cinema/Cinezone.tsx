...
function Cinezone() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch("http://localhost:5000/api/Cinema/getCinemaList")
            .then((res) => {
    ...
    useEffect(() => {
        const fetchInShowMovies = async () => {
            setLoading(true);
            try {
                const response = await fetch("http://localhost:5000/api/movie/GetInShowedMovie");
    ...
    useEffect(() => {
        const fetchUpcomingMovies = async () => {
            setLoading(true);
            try {
                const response = await fetch("http://localhost:5000/api/movie/GetUnShowedMovie");
                if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
                const result: ApiResponse = await response.json();
                if (result.status === "Success") {
                    const formattedMovies = result.data.map((item) => ({
